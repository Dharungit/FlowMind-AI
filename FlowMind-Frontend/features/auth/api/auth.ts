      ...
      if (account?.id_token) {
        try {
          const res = await fetch(`${API_URL}/v1/auth/google`, {
            ...
            body: JSON.stringify({ id_token: account.id_token } satisfies { id_token: string }),
          })
      ...
      if (token.expiresAt && Date.now() > token.expiresAt && token.refreshToken) {
        try {
          const res = await fetch(`${API_URL}/v1/auth/refresh`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ refresh_token: token.refreshToken } satisfies { refresh_token: string }),
          })
